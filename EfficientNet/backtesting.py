...
def backtest( start, end, model_list, fore, topX,   sellProfit, kos, label_list):
    ...
    count=0
    # #/////////////////////
    # df = data.get_data_yahoo("^KS11", int(start_date), int(end_date))
    # stock_data_label = 'stcok'
    # if(kos=='Kosdaq'):
    #   df = data.get_data_yahoo("^KQ11", int(start_date), int(end_date))
    ...
    for mod in range(len(model_list)):
      ...
      plt.plot(d_list, no_list, color_for_com[mod], label= label_list[mod])
      kospi_check  = True
      date_check = True
    ...
    plt.plot(kospi_date_list, kospi_list, 'g', label= kospi_label)
    # plt.xlabel('Date')
    # plt.ylabel('Profit')
    ...
    for i in range(len(model_count)):
      profit_rate = int((win_list[i]/(win_list[i]+lose_list[i])) *100)
      ...
      plt.text(-10,model_count[i]-0.01 ,label_list[i], color="black",fontsize=15)
